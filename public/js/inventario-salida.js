document.addEventListener('DOMContentLoaded', () => {
    ...
    function procesarEscaneo(codigo) {
        ...
        if (productoActual && cantidadAcum > 0) {
            confirmarVenta(() => buscarNuevo(codigo));
            return;
        }
        buscarNuevo(codigo);
    }
        ...
        .then(data => {
            if (data.encontrado) {
                if (data.producto.stock <= 0) {
                    ...
                    mostrarMensaje('Sin stock disponible para este producto.', 'error');
                ...
                productName.textContent  = data.producto.nombre_producto;
                productStock.textContent = 'Stock disponible: ' + data.producto.stock;
                ...
                if (data.producto.imagen) {
                    ...
                    productImg.innerHTML = '<i class="fa-solid fa-cart-shopping"></i>';
                }
    ...
    function reiniciarTimer() {
        ...
        timerConfirmar = setTimeout(() => confirmarVenta(), DELAY_MS);
    }

    function confirmarVenta(callback) {
        ...
        fetch(BASE_URL + '/inventario/vender', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        ...
        .then(data => {
            if (data.ok) {
                mostrarMensaje('Venta registrada: −' + qty + ' unidad(es). Stock: ' + data.producto.stock, 'ok');
            } else {
